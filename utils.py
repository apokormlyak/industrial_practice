...
def show_workers(to_show: List[Worker]) -> None:
    """
    Функиця отображает список сотрудников, подходящих по условию отображения
...
def get_option() -> int:
    """
    Функиця взаимодействует с пользователем, предлагая выбрать один из трех сценариев
